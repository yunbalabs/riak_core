...
transfer_summary() ->
    ...
    {Summary, DownNodes} = build_transfer_summary(ongoing_transfers_summary(), Ring),
    ...
    Table = {table, Schema,
        [ [ format_node_name(Node, DownNodes), format_summary(S) ] || {Node, S} <- orddict:to_list(Summary) ]},
    [Header, Table].
...
build_transfer_summary({OngoingSummary, DownNodes}, Ring) ->
    Outstanding = [ outstanding_count(T, Ring) || T <- [ownership_transfer, hinted_handoff, resize_transfer, repair] ],
    {orddict:map(fun(Node, D) -> merge_outstanding(Node, D, Outstanding) end, OngoingSummary), DownNodes}.

merge_outstanding(Node, Ongoing, Outstanding) ->
    [OO, OH, ORz, ORp] = lists:map(fun(L) -> proplists:get_value(Node, L, 0) end, Outstanding),
    orddict:map(
      fun(K, V) -> 
        case K of
        ownership_transfer -> {V, OO-V , OO };
            hinted_handoff -> {V, OH-V , OH }; 
           resize_transfer -> {V, ORz-V, ORz}; 
                    repair -> {V, ORp-V, ORp}
        end
      end, Ongoing).
...
ongoing_transfers_summary() ->
    ...
    {Ongoing, DownNodes} = riak_core_util:rpc_every_member_ann(riak_core_handoff_manager,
                                                                status,
                                                                [{direction, outbound}],
                                                                5000),

    Summary = lists:foldl(fun build_ongoing_node_summaries/2, orddict:new(), Ongoing),
...
build_summaries(OutboundTransfers) ->
    I = orddict:from_list([{ownership_transfer, 0}, {hinted_handoff, 0}, {resize_transfer, 0}, {repair, 0}]),
